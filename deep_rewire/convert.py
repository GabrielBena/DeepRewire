...
def register_params(
    module: nn.Module,
    weight_signs: torch.Tensor,
    bias_signs: torch.Tensor = None,
    bias_negative: torch.Tensor = None,
):
...
def get_signs(
    ...
    if active_probability is not None:
        ...
        active_tensor = (
            torch.bernoulli(torch.full(w.size(), active_probability, device=device)) * 2
            - 1
        )
        if hasattr(module, "parametrizations"):
            w = torch.abs(module.parametrizations.weight.original)
            module.parametrizations.weight.original = nn.Parameter(w * active_tensor)
        else:
    ...
    if handle_biases == "as_connections":
        ...
        if active_probability is not None:
            ...
            active_tensor = (
                torch.bernoulli(torch.full(b.size(), active_probability, device=device))
                * 2
                - 1
            )
...
def convert(
    ...
    def linear_forward(x, mod=module):
        ...
        if handle_biases == "second_bias":
            bias = (
                F.relu(mod.bias) - F.relu(mod.bias_negative)
                if mod.bias is not None
                else None
            )
            return F.linear(x, F.relu(mod.weight) * mod.weight_signs, bias)
        raise ValueError(f"{handle_biases} is not a valid option for handle_biases")
    ...
    def conv2d_forward(x, mod=module):
        if handle_biases == "ignore":
            if mod.padding_mode != "zeros":
                return F.conv2d(
                    F.pad(
                        x, mod._reversed_padding_repeated_twice, mode=mod.padding_mode
                    ),
    ...
    for name, submodule in module.named_children():
        if filter is not None and name in filter:
            continue
        sparse_p, other_p = convert(
            submodule,
            handle_biases=handle_biases,
            active_probability=active_probability,
            keep_signs=keep_signs,
        )
...
def merge_back(module: nn.Module):
    ...
    parameters_to_merge = [
        (n[:-6], n) for n in module.state_dict() if n.endswith("_signs")
    ]
    for p_name, s_name in parameters_to_merge:
        merge_signs(p_name, s_name)

    parameters_to_merge = [
        (n[:-9], n) for n in module.state_dict() if n.endswith("_negative")
    ]
    for p_name, s_name in parameters_to_merge:
        p_hierarchy = p_name.split(".")
        ...
        setattr(
            obj,
            p_hierarchy[-1],
            torch.nn.Parameter(value.clamp(min=0) - neg.clamp(min=0)),
        )
...
def forward_to_standard(module: nn.Module):
    ...
    if isinstance(module, nn.Linear):
        ...
        module.forward = lambda x, mod=module: (
            F.conv2d(
                F.pad(x, mod._reversed_padding_repeated_twice, mode=mod.padding_mode),
                mod.weight,
                mod.bias,
                mod.stride,
                _pair(0),
